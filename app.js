...
var memcached = new Memcached('my-memcached.yet1ke.cfg.apse1.cache.amazonaws.com:11211');
...
var con = mysql.createConnection({
  host: "database-1.cbqa7a44lrht.ap-southeast-1.rds.amazonaws.com",
  user: "user",
  password: "Test1234",
  database: "mydb"
});
...
app.get("/home", function(request, response) {
  ...
  if (itemslist.length == 0) {
    con.query("SELECT * FROM blogstable", function (err, result, fields) {
      ...
      result.forEach(function(myitem) {
         memcached.set(myitem.blogtitle, myitem.blogbody, 500, function (err) {  });
         itemslist.push(myitem.blogtitle);
        });
    });
   } else {
     itemslist.forEach(function(item) {
       memcached.get(item, function (err, data) {
        results1.push({'blogtitle':item, 'blogbody':data});
       });
      });
...
//   });
  memcached.get(enteredtitle, function (err, data) {
     res.render("post", {newposttitle: enteredtitle, newpostbody: data});
   });
});
